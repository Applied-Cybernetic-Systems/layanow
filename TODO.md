...
- **T-179** · 2026-09-23 · app — Let the user supply context (Laya's `state`): the app always sent `{}` (ADR-23), but with the relevant passage in `state` the same question goes from 37% (wrong) to 92% (correct). **Closed:** 2026-09-23 · added an optional **Context** box to the overlay (Option A): type/paste, read a `file://` selection from a file manager, or pick/save a named context template (`Settings.contexts`, managed in the settings window). The context is sent as `state` `{"context": …}` via `render::context_state`; question stays `ins`, answers `crit`. The capturing panel became pointer-interactive so the box/dropdown/Save/Files controls work. A **Files…** button opens the native picker (`rfd` → xdg-desktop-portal FileChooser) on a worker thread. Shipped in the `feat(app): optional context capture, files and templates (T-179)` commit (ADR-40).
