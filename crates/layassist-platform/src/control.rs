...
//!
//! `layassist toggle` (and `show`/`hide`/`quit`) talk to the resident applet
...
pub const DEFAULT_ID: &str = "layassist";
...
pub enum ControlError {
    ...
    #[error("another layassist instance is already running")]
    AlreadyRunning,
    /// No applet is listening on the control socket.
    #[error("no running layassist instance ({0})")]
    NotRunning(String),
...
fn start_with(id: &str) -> Result<Receiver<Command>, ControlError> {
    ...
    thread::Builder::new()
        .name("layassist-control".to_string())
