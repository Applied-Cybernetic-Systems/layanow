...
//!
//! The trait itself lives in `layassist-resolvers`; the OS calls live here so
...
impl TextResolver for UnavailableResolver {
    ...
    fn resolve_current_selection(
        &self,
    ) -> Result<Option<layassist_core::Selection>, layassist_resolvers::ResolveError> {
        Ok(None)
    }

    fn watch(&self) -> Option<layassist_resolvers::SelectionStream> {
        None
    }
}
