...
//! and the control socket listens for `layassist toggle`/`show`/`hide`/`quit`
...
//! (`layassist-platform::selection`, M4).
...
use layassist_platform::control::{self, Command, ControlError};
...
fn print_help() {
    println!(
        "layassist — local multiple-choice assistant\n\
         \n\
         Usage:\n\
           layassist            run the resident applet (overlay starts hidden)\n\
           layassist toggle     show/hide the overlay\n\
           layassist show       show the overlay\n\
           layassist hide       hide the overlay\n\
           layassist quit       stop the running applet\n\
           layassist --help     print this help"
    );
}
...
fn run_applet() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // makes `layassist toggle` responsive while the model loads.
    let commands = match control::start() {
        Ok(commands) => commands,
        Err(ControlError::AlreadyRunning) => {
            eprintln!("layassist: already running");
            return Ok(());
        }
    ...
    let result = layassist_platform::overlay::run(Overlay::new(worker, resolver, commands));
