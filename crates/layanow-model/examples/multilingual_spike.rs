...
//!
//! Point `LAYANOW_M1_BUNDLE` at an export directory containing `laya.onnx`,
//! `laya_config.json` and `tokenizer/` (default `target/m1-export/onnx-multilingual`).
//! Set `LAYANOW_M1_QUANT=int8` to load an int8 graph named `laya.onnx` there.
//!
//! Run it under `/usr/bin/time -v` to read the resident memory:
//!
//! ```sh
//! nix develop
//! LAYANOW_M1_BUNDLE=target/m1-export/onnx-multilingual \
//!   /usr/bin/time -v cargo run -p layanow-model --example multilingual_spike
//! ```
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let dir = std::env::var("LAYANOW_M1_BUNDLE")
        .map_or_else(|_| PathBuf::from("target/m1-export/onnx-multilingual"), PathBuf::from);
    let quant = match std::env::var("LAYANOW_M1_QUANT").as_deref() {
        Ok("int8") => Quant::Int8,
        _ => Quant::Fp32,
    };
